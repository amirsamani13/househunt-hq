-- Fix user notifications and add real-time triggers
UPDATE public.profiles 
SET notifications_paused = false, updated_at = now()
WHERE email = 'amirsamani13@gmail.com';

-- Create real-time notification trigger function
...
DROP TRIGGER IF EXISTS trigger_notify_matching_users ON public.properties;
CREATE TRIGGER trigger_notify_matching_users
  AFTER INSERT ON public.properties
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_matching_users();

-- Set up cron job for continuous scraping every 5 minutes
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    ...
    headers := '{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im94ZG5laWFvamd3ZXp4bHRpdmNsIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MzcwNjk4NSwiZXhwIjoyMDY5MjgyOTg1fQ.WnvHGDQ-xJfT9eo51EbLmPfUqPNMPglLhkPWN7lWsOA"}'::jsonb,
    body := '{"sources": ["pararius", "kamernet", "grunoverhuur", "funda", "kamernet"], "autoRepair": true}'::jsonb
  );
  $$
);

-- Set up cron job for auto-repair every 30 minutes
SELECT cron.schedule(
